...
View(book)
class(book)
# since the data is in list we are converting it into trasations, so before
# that need to convert into matrix format
book_trans <- as(as.matrix(book), "transactions")
View(book_trans)
# incase of list format directly can be called in transactions.
inspect(book_trans)
...
a <- read.csv("C:/EXCELR/ASSIGNMENTS/Association_rules/groceries.csv")
View(a)
groceries <- read.csv("C:/EXCELR/ASSIGNMENTS/Association_rules/groceries.csv")
View(groceries)
length(groceries)
str(groceries)
...
#3.) my movies
my_movies <- read.csv("C:/EXCELR/ASSIGNMENTS/Association_rules/my_movies.csv")
View(my_movies)
str(my_movies)
colnames(my_movies)
